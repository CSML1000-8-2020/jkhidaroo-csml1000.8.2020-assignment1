...
Simple algorithm for Bayesian classification which ends up being counts over all the observations. It works on a principle that all features are required to predict a model and are all correlated. As the name suggested it is based on Bayesian theorem, hence this algorithm is good for dataset with high dimensionality and high number of classes. It uses Bayesian theorem as the heart of its algorithm to solve the problem at hand
 
Advantages:
Training time for computations quite less.
Removal of all  the irrelevant features from the dataset
Improved Performance
 
Disadvantages:
 · Requires a very large number of records
 · Less accuracy
 
Features of Decision Trees: 
 Decision trees are part of tree structured algorithm consisting of leafs, nodes and roots as their attributes. Top node is called root consisting of attributes and branching factors which other nodes. A Major Disadvantage over other classifiers is that it can easily overfit.
Advantages:
 · Simple and fast.
· Highly Accurate, Scalable
· It takes the less memory.
 · Works with Noisy Data/Outliers
· Works with measures like Entropy, Gini index, Lift etc.
Works with hyperparameters.
 
Disadvantages:
· Long training time.
· Issues with overfitting in certain cases
·      Not Reusable
...
Disadvantage:
Although it is used frequently in text classification  and it often deals with very high-dimensional spaces.
Memory-intensive operationally
Hard to understand and implement certain .
  
  The best features for our dataset for our neural network resulted in a mean squared error on our training set of 97.  This trained model was then applied to the test dataset, in attempt to predict the one-year return variable (Output Return %) given the 16 data inputs.  The mean squared error for the test set prediction was 236.88, and the corresponding R2 value was -0.0071.   An R2 value of approximately 0 indicates that this model shows no predictive capabilities for the future value of stocks.
Despite applying a neural network (i.e., potentially allowing for any function to explain the data), with optimized hyperparameters, to over 569 observations (426 training and 142 testing) of firms with data on 32 fundamental and technical features. since the features were all numerical and continous it helped us to get accurate prediction because of cleansed dataset.
