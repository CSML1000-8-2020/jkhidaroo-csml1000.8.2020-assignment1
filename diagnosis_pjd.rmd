...
diagnosis_data_train <- diagnosis_data_full[1:400,]
diagnosis_data_test <- diagnosis_data_full[401:500,]
```
...
diag_nb_model <- naiveBayes(diagnosis_data_train[,-c(1)], diagnosis_data_train$diagnosis)
```
...
```{r, message = FALSE}
diag_nb_pre <- predict(diag_nb_model, diagnosis_data_test[,-c(1)])
...
```{r, message = TRUE}
pre_mean_rf <- predict(diag_mean_model, diagnosis_data_test[,-c(1)])
...
```{r, message = FALSE}
pre_se_rf <- predict(diag_se_model, diagnosis_data_test[,-c(1)])
...
```{r, message = FALSE}
pre_worst_rf <- predict(diag_worst_model, diagnosis_data_test[,-c(1)])
...
```{r, message = FALSE}
pre_mean_worst_rf <- predict(diag_mean_worst_model, diagnosis_data_test[,-c(1)])
...
```{r, message = FALSE}
diag_lg_pre<-predict(diag_lg,diagnosis_data_test[,-c(1)])
...
#### Evaluation of the kFolds SVM Model

```{r, message = FALSE}
folds <- createFolds(diagnosis_data_train$diagnosis, k = 10)
cv <- lapply(folds, function(x){
  training_fold = diagnosis_data_train[-x, ]
  test_fold = diagnosis_data_train[x, ]
  diag_kSVM_model <- svm(formula = diagnosis_data_train$diagnosis ~.,
                 data = training_fold[,-c(1)],
                 type = 'C-classification',
                 kernel = 'radial')
  diag_kSVM_pre <- predict(diag_kSVM_model, test_fold[,-c(1)])
  diag_kSVM <- confusionMatrix(test_fold[,-c(1)], diagnosis_data_test$diagnosis)
  accuracy = (diag_kSVM[1,1] + diag_kSVM[2,2]) / (diag_kSVM[1,1] + diag_kSVM[2,2] + diag_kSVM[1,2] + diag_kSVM[2,1])
  return(accuracy)
})
